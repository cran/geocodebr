...
#'
#' @export
geocode_reverso2 <- function(pontos,
                            dist_max = 1000,
                            verboso = TRUE,
                            cache = TRUE,
                            n_cores = 1){
  ...
  unique_munis <- paste0(potential_munis, collapse = ", ")
  ...
  query_unique_logradouros <- glue::glue_sql(.con = con,
    "CREATE OR REPLACE TEMP TABLE filtered_cnefe AS
          SELECT DISTINCT {cols_to_keep}
